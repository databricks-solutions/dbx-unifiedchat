...
def get_config(reload: bool = False) -> AgentConfig:
    ...
    Apps path:       .env + env vars → AgentConfig.from_env()
    Databricks path: YAML → ModelConfig → AgentConfig.from_model_config()
    ...
    if _config is None or reload:
        if is_databricks_apps():
            print("Detected Databricks Apps environment. Loading config from env vars...")
            if reload:
