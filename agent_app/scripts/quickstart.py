...
def _check_autoscaling_project_exists(profile_name: str, project_name: str) -> dict | None:
    """Check if an autoscaling Lakebase project already exists.

    Returns the project info dict if it exists, None otherwise.
    """
    result = run_command(
        [
            "databricks", "-p", profile_name,
            "api", "get", f"/api/2.0/postgres/projects/{project_name}",
            "--output", "json",
        ],
        check=False,
    )
...
def _check_provisioned_instance_exists(profile_name: str, instance_name: str) -> dict | None:
    """Check if a provisioned Lakebase instance already exists.

    Returns the instance info dict if it exists, None otherwise.
    """
    result = run_command(
        [
            "databricks", "-p", profile_name,
            "database", "get-database-instance", instance_name,
            "--output", "json",
        ],
        check=False,
    )
...
def create_lakebase_instance(profile_name: str) -> dict:
    ...
    Checks for conflicts before creating. If a project with the same name
    already exists, offers the user options to reuse it or pick a different name.
    ...
    while True:
        ...
        existing = _check_autoscaling_project_exists(profile_name, name)
        if existing:
            print(f"\n  A Lakebase project named '{name}' already exists.")
            print("  Options:")
            print("    1) Use the existing project (reuse it)")
            print("    2) Enter a different name")
            print()

            while True:
                conflict_choice = input("  Enter your choice (1 or 2): ").strip()
                if conflict_choice in ("1", "2"):
                    break
            ...
            if conflict_choice == "1":
                ...
                result = run_command(
                    [
                        "databricks", "-p", profile_name,
                        "api", "get", f"/api/2.0/postgres/projects/{name}/branches",
                        "--output", "json",
                    ],
                    check=False,
                )
                ...
                if result.returncode == 0 and result.stdout.strip():
                    try:
                        ...
                        if branches:
                            raw = branches[0].get("name", "")
                            branch_name = raw.split("/branches/")[-1] if "/branches/" in raw else branch_name
                    except (json.JSONDecodeError, IndexError, KeyError):
                        pass
                print_success(f"Reusing existing project: {project_short} (branch: {branch_name})")
                return {"type": "autoscaling", "project": project_short, "branch": branch_name}
            # choice == "2": loop again to ask for a new name
            continue
