...
def get_table_metadata(table_identifier: str) -> pd.DataFrame:
    ...
    """
    try:
        df_description = spark.sql(f"DESCRIBE {table_identifier}") # for simplicity, removed keyword 'DESCRIBE EXTENDED' from SQL query. this saves all the hassles of below filtering.
...
def process_genie_space(space_json_path: str, enriched_docs_table: str) -> Dict[str, Any]:
    ...
    # Synthesize space description and append to existing description if any
    space_description = space_data.get('description', '')
    if enriched_tables:
        print(f"\n→ Space description is improving. Synthesizing from table metadata and appending to existing description if any...")
        space_description += "\n------LLM improved version is below------\n"
...
for chunk_type in ['space_summary', 'space_details', 'table_overview', 'column_detail']:
    print(f"\n{chunk_type.upper()}:")
    sample = spark.table(chunks_table_name).filter(f"chunk_type = '{chunk_type}'").limit(3)
...
# MAGIC
# MAGIC Next: Use these chunks to build vector search index (`03_VS_Enriched_Genie_Spaces.py`)
# MAGIC
